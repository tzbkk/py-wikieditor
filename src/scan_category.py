#!/usr/bin/env python3
"""
扫描并转换所有 category 命名空间页面

功能：
1. 扫描所有 category 命名空间（namespace=14）的页面
2. 检测哪些页面需要转换（包含繁体中文）
3. 显示每个页面的更改预览
4. 逐个询问是否批准保存修改
"""
...
def scan_category_namespace(bot: FandomBot, limit: Optional[int] = None) -> List[str]:
    ...
    try:
        for page in bot.site.allpages(namespace='14'):
            count += 1
...
示例:
  ...
  %(prog)s --scan-only
    ...
    # 扫描 category 命名空间
    pages_to_convert = scan_category_namespace(bot, args.limit)
